...
- [ ] C-x 2 window split (window-splits lane, 2026-09-20): the single-pane
      view-stack model — C-x 2 is BOUND but reports ("single pane: … scoped
      follow-up"), no state change; a real vertical split needs per-pane
      buffer/point/scroll state + a second render pane + window-focus
      cycling. C-x 0 / C-x 1 degrade to stack semantics (close top view /
      truncate to the buffer view; no-ops on the single view)
...
| 2026-09-20 | window-splits lane (C-x 2/1/0) | sized-pty 80×24 (pyte) + cargo | win-splits branch | **ALL PASS** | The 3 pre-existing ux_sweep findings (C-x 2/1/0 unbound) **cleared**: model decision (b) — view-stack-degraded semantics (no split layout: redline has a view stack, single current buffer/point/scroll; a real vertical split is a renderer + per-pane-state change = DECIDED OUT (see the U-K row)). C-x 0 → `close-view` (closes the top view; no-op on the last view), C-x 1 → new `close-other-views` (truncates the stack to `[Buffer]`; no-op on the single view), C-x 2 → new `split-window-vertical` (reports "single pane by design: redline runs as a herdr popup, one pane", zero state change) — all bound on the buffer view only (home keeps 06a's empty keymap; non-file views echo unbound). Unit twins (state + render80, loop-03): `unit_flow_window_splits_c_x_0/1/2_*` incl. degradations (C-x 0 on the last view + home, C-x 1 with one view, C-x 2 of a non-file view); `unit_flow_ux_keymap_coverage`'s known-unbound set is now EMPTY. Gates: cargo test **814 passed / 0 failed / 2 ignored** (811 + 3 new); clippy `--all-targets -- -D warnings` clean; ux_sweep.py window-splits leg: 0 unbound echoes (was 3 findings). Palette 106 → 108 (the 2 new seed commands). |
| 2026-09-20 | burst+release coalescing (NEW, input-latency lane) | sized-pty 80×24 (pyte) + cargo | input-lat branch (1f0b4ef + review P2s) | **ALL PASS** | Input-latency lane (queued/repeating held-key input over a stream — user report: motion keeps replaying after release). Loop shape: every crossterm event wakes the render loop, which drains ALL queued events in one poll pass and applied each queued repeat to the store individually (N repeats = N re-parse/re-render passes; post-release replay). Fix = drain-and-coalesce at the app event boundary (new `src/ui/event_loop.rs`, wired into the `Root` callback + render tick; store.rs and main.rs untouched — no store API needed): motion keys (arrows + PgUp/PgDn unmodified, C-n/C-p/C-v/M-v — NEVER char keys, which double as text/picker input, and never sequence prefixes) coalesce last-wins into one pending slot (`InputCoalescer`); state-changing keys apply individually, flushing the pending motion FIRST (ordering); the render tick flushes the pending motion BEFORE the store snapshot — a burst applies at most once per drain pass and after a release at most the one pending motion flushes, so post-release replay is structurally impossible; a single keypress still applies exactly once (byte-for-byte). Repro pins: event_loop unit tests (100 queued C-n → exactly 1 apply; release-drain: 0 applies across 10 idle ticks after the flush; C-n,C-n,C-f → [C-n,C-f]; mixed last-wins; single-keypress == direct apply) + root.rs store-level twins (50 queued C-n → exactly 1 `key_event`, final point == one direct C-n; ordering leg). **New PTY leg** (`check_cursor_stream.py`, BURST+RELEASE section): 30 C-n in ONE pty write → point advanced a BOUNDED step — asserts the WIDE 2..=5 line range, because the discriminating property is "bounded, not ~30" (pre-fix: L30 / CUP row 22, far outside) and the exact per-drain-pass apply count is load-sensitive and unit-pinned (event_loop motion tests + root.rs burst twin), so a PTY leg must not couple to the exact pass count (review P2 — the first version's exact `(2,3,4)` bound was a load-flake risk); release-drain: no motion across two post-burst quiet windows; paced single C-n: exactly one line per press. **Two pacing deviations (harness-only, forced by the new coalescing semantics; app behavior for paced presses is byte-for-byte identical)**: (1) check_cursor_stream's `C-n x4` leg paced to 4 single presses (a 4-key burst now coalesces by design; that leg's position chain needs exactly 4 lines — the burst itself is asserted by the new leg); (2) probe_notes_dump's `C-n C-n` (OUTSIDE the lane's literal file fence — orchestrator-acknowledged as gate-necessary: the burst would have landed the live annotation on line 1 instead of line 2) paced to two presses. **LESSON FOR FUTURE PTY AUTHORS: never burst-write motion keys (C-n/C-p/C-v/M-v/arrows/PgUp/PgDn) in one `key()` call — post-coalescing a burst is ONE apply per drain pass by design; pace individual presses when a leg needs N applies, and when a leg's point IS the burst, assert the bounded range ("not N"), not the exact count.** Gates: cargo test --workspace **833 + 120 + 7 + … passed / 0 failed** (2 pre-existing ignored); clippy `--workspace --all-targets -- -D warnings` clean (PIPESTATUS 0); **tools/gate.sh full: exit 0, 20/20 steps OK** (check_cursor_stream.py 84/84 incl. the 4 new burst/release checks; probe_notes_dump.py 17/17; sweep_flows 15/15; all 17 PTY suites green). |
...
| `ux_sweep.py` (13 fresh Apps → 2) | the per-key anomaly scan (unbound echoes, blank frames, overflow rows, off-screen cursor) in one 80-col session + one 40-col narrow session | `unit_flow_ux_keymap_coverage` (every key of every leg through `key_event`; the window-split keys C-x 2/1/0 are now BOUND — no parity-pinned unbound keys remain; their behavior lives in `unit_flow_window_splits_c_x_0/1/2_*`) |
