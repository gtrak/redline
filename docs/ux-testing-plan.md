...
- [ ] **`git::repo` flake: swap-exhaustion signature (deflake-timing item 3)**. `stage_file_then_unstage_matches_cli` failed under **two concurrent full suites** with **no timing/ordering assumption to pin**: the wrapper keeps no index cache, and a git-level probe ruled out the libgit2 stat short-circuit (`dev=0` vs the real `st_dev=30` forces a re-hash). It correlates with a **fully-exhausted swap** (8 G/8 G used) during the repro. Policy: **(1) NO auto-retry** (the gate suggested a single auto-retry; this overrides that suggestion with this reason) — an automatic retry converts a *detectable* environmental failure into an *invisible* one, which a gate must not do; **(2) resource-guard wording**: a lane must not start a full battery when swap is exhausted — the guard is `free -g` "available" **and swap used** (not `free -g` alone), and when swap is full the lane runs the targeted loops only and reports the battery as deferred (no sleep-wait); **(3) full panic text on failure** — a failing run captures the FULL panic output (backtrace included), never tail-truncated, so the next occurrence is attributable.
- [ ] **Third flake class: unattributed external signal-kill of the `test` stage (store-tests gate, `gate.sh full`)**. The first full run's cargo `test` stage was SIGTERM-killed at ~test 812/847 — no OOM-kill in the journal, every PTY stage still ran and passed, and the re-run was 21/21 green. Three mechanisms were eliminated (the gate wraps only the PTY steps in `timeout`; the kill came at 78 s so no per-tool deadline applied; the PTY harness only ever SIGKILLs its OWN spawned app, never by name, never SIGTERM) — so it is an unattributed external kill: a distinct flake class, not the cursor race and not the `git::repo` assertion flake. Rule (refines item 3 above): **auto-retry is wrong for a real test failure but legitimate for a signal-kill** — a process killed by signal is not a test outcome at all. Any retry therefore must be **signal-specific and logged**: trigger only on cargo exit 143 (SIGTERM) / 137 (SIGKILL), print e.g. "test stage killed by signal N — retried once", and **never** trigger on an assertion failure (exit 101). No blanket retry. Note: `tools/gate.sh` is outside the deflake-timing fence, so this lane documents the rule only — implementing it is a follow-up in the gate's own lane.
