...
| # | Row | Disposition | Evidence |
|---|---|---|---|
| 8 | C-l recenter | **CONTRACT REPLACED (05c)** | The battery-adopted model (cycle the WINDOW through buffer thirds, cursor-is-window-top) misbehaved: the window scrolled off the point (observed: point line 20, window top 47). 05c implements emacs `recenter-top-bottom`: the point does not move; its SCREEN ROW cycles top → middle → bottom → top within the viewport (`scroll_top = point_line − {0, viewport/2, viewport−1}`, clamped to `[0, max_scroll]`). Tiny viewports keep cycling via the reworked `third.max(1)` protection; barely-scrolling buffers collapse to the reachable positions (same as emacs). Unit tests reworked (not deleted): `recenter_from_{bottom,top,middle}_zone_to_{top,middle,bottom}`, `recenter_full_cycle_returns_to_start`, `recenter_tiny_scroll_ranges_keep_the_point_in_view`, `recenter_tiny_viewports_still_cycle`; `recenter_noop_when_buffer_fits_viewport` retained. The `cursor_cell` hardware-cursor row is clamped to the viewport so a window that sits off the point can never place the cursor outside the file area. |
| word | M-f / M-b | **IMPLEMENTED (05c)** | `word-forward` / `word-backward` registered (motion); `M-f` / `M-b` bound in the Buffer view keymap. Fixed rule (documented on `is_word_char` in store.rs): word = run of alphanumeric/`_`; non-word = every other character (punctuation AND whitespace, one class); newlines are non-word, so the skip crosses line boundaries exactly like emacs `skip-chars-forward/backward non-word-constituent`. Difference from the emacs syntax table: emacs's word-constituent set is per-buffer/configurable (e.g. `?`/`!` may be word-constituents; symbol vs whitespace categories differ) — redline's is a fixed class. Word motion sets `goal_col` to the landing column (emacs). Unit tests: `word_forward_walks_words_and_punctuation`, `word_backward_walks_words_and_punctuation`, `word_motion_over_empty_lines`, `word_motion_sets_goal_column_to_landing_col`. |
