...
`unverified by worker — orchestrator spot-check`. (None at time of
writing: every cell carries a code line, unit test, or drive leg.)
...
| **Tsx** | works (unit) — shares `TYPESCRIPT_QUERY` (`query_for`, queries.rs:251); TSX-specific extraction test name: unverified by worker — orchestrator spot-check | works (unit) — same shared machinery; node.rs ts tests loop TS+Tsx (`ts_member_path_comes_back_whole`); no live leg | works (unit) — `js_ts_scope_for` covers Tsx (`resolver_scope_for`, store.rs:8169); no Tsx-specific test name verified | works (unit) — `ts_scope_chain_function_class_method_arrow` (loops TS+Tsx) | `js/jsx/ts/tsx/mjs/cjs` (shared with JS/TS) | `JsProvider` (`"tsx"` in `languages()`) | works (unit) — same shared js-tree index; landing leg: unverified by worker — orchestrator spot-check | works — `all_grammars_have_configs` |
