...
| `A` | annotate | Annotate at point (minibuffer prompt; RET commits — a record in `.redline-notes.md`, the inline cue appears immediately; a record at the point's cell pre-fills for edit, and a point with no record of its own creates one, even on a line that already has annotations) |
| `d` | annotate-delete | Delete the annotation at point (echoes the removed note; a message when the point addresses no annotation) |
