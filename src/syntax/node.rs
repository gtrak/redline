...
fn parse_source(lang: LanguageId, source: &str) -> Option<tree_sitter::Tree> {
    // Reuse the registry's pinned grammar constant (the same `LanguageFn`
    // `registry.rs` builds its `Language` from) — no second registry.
...
fn rust_scope_path(leaf: Node, source: &[u8]) -> Vec<String> {
    ...
    while let Some(node) = cur {
        let field = match node.kind() {
            "mod_item" | "trait_item" | "function_item" => Some("name"),
            "impl_item" => Some("type"),
            _ => None,
        };
...
mod tests {
    ...
    fn top_level_code_has_empty_scope_path() {
        ...
        // `g` itself is a scope item for code inside it, but not for
        // itself.
