...
impl CommandRegistry {
    ...
    pub fn seed() -> Self {
        ...
        reg.register_navigation();
        reg.register_scratch();
        reg.register_find_file();
        reg.register_buffer_switching();
        reg.register_project();
        reg.register_project_files();
        reg.register_motion();
        reg.register_isearch();
        reg.register_view_management();
        reg.register_buffer_list();
        reg.register_magit();
        reg.register_commit_diff_motion();
        reg.register_blame();
        reg.register_watching();
        reg.register_symbol_navigation();
        reg.register_search();
        reg.register_tree();
    ...
    fn register_find_file(&mut self) {
        self.register(Command::new(
            "find-file",
    ...
    fn register_buffer_switching(&mut self) {
        self.register(Command::new(
            "switch-buffer",
    ...
    fn register_project_files(&mut self) {
        self.register(Command::new(
            "recent-files",
    ...
    fn register_view_management(&mut self) {
        self.register(Command::new(
            "close-view",
    ...
    fn register_buffer_list(&mut self) {
        self.register(Command::new(
    ...
    fn register_commit_diff_motion(&mut self) {
        self.register(Command::new(
    ...
    fn register_buffer_editing(&mut self) {
        self.register(Command::new(
            "open-notes",
...
mod tests {
    ...
    fn dispatch_by_name_runs_the_command() {
        ...
        let mut store = new_store();
        reg.dispatch_by_name(&mut store, "open-scratch", None)
            .unwrap();
    ...
    fn dispatch_unknown_command_errors() {
        ...
        let mut store = new_store();
        let err = reg
            .dispatch_by_name(&mut store, "does-not-exist", None)
            .unwrap_err();
    ...
    fn view_commands_work() {
        ...
        reg.dispatch_by_name(&mut store, "list-buffers", None)
            .unwrap();
    ...
    fn readme_keymap_table_matches_registry() {
        let readme =
            std::fs::read_to_string("README.md").expect("README.md must exist at the repo root");
        let reg = CommandRegistry::seed();
        let registry_names: std::collections::HashSet<&str> = reg.list().map(|c| c.name).collect();
