...
1. `.agents/plans/005-edit-modes/03-quit-dump.md` — contract + pre-check
   ...
   store is behind `store_handle: Arc<Mutex<AppStore>>`; stdout is clean).
...
3. **Emit after teardown**: after `app.fullscreen().ignore_ctrl_c().await?`
   ...
   lock the store handle and print the dump to stdout. Never print before
   that point (must not interleave with the TUI).
