...
| `issue-annotations-render-fold` | OPEN (specced) | spec written from the user's decisions; no code yet | the user's decisions: **margin indicator**, note **ABOVE** the anchored line (today it is below, dim+italic, truncated), **folding**, and the tree **`C-c a`** — `n`/`h`/`s`/`l`. `C-a` was **explicitly declined** (it is emacs's `point-line-start`; a prefix cannot also be a command). Shift-to-fold is **conditional**: crossterm can represent a bare modifier only for kitty-protocol keycode 57441, so the lane must first verify whether enhancement is even enabled and must **not** fight the framework if it is not — a binding that silently never fires on the user's terminal is worse than none |
