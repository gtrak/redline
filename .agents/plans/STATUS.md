...
| Spec | State | Evidence | Notes |
|---|---|---|---|
| `issue-deps-sweep` | PARTIAL | batch A landed **`d665ce3`** (8 semver-compatible transitive bumps; `cargo build` clean, workspace tests 10 suites exit 0, clippy 0, `gate.sh full` **18/18 flows**) | swept all 43 declared deps against the cached crates.io index: **5 were behind**, and the rest are deliberate or already done. **HELD on purpose — do not re-litigate:** `tree-sitter` + `tree-sitter-highlight` (`=0.25.10`) and `tree-sitter-md` (`=0.5.1`) — `docs/tree-sitter-runtime-matrix.md` records the verdict that the grammars wanting `^0.26` *"remain outside the ABI window and are NOT bumped"*, and the manifest comment says 0.5.2+ want `^0.26`, so the md pin **cannot** move while the runtime is 0.25.10. **tokio was already `1.53.1`** in the lock, so the sweep's "1.53 → 1.53.1" needed no action at all. **The one actionable bump left: `unicode-width` 0.1.14 → 0.2.2** — one call site (`src/model/text_width.rs:11`, `UnicodeWidthChar::width()`), but it changes the width *tables*, i.e. what counts as double-width, so it needs behavioural verification (the wide-char tests + a spot-check of ambiguous-width chars), not just a compile. Also note the sweep's own comparison had to be corrected: naive semver keys made `"0.29"` vs `"0.29.0"` read as *behind* when they are the same version |
