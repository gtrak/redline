...
| `issue-annotations-render-fold` | IN FLIGHT (repair) | lane slot 3, `annotations-fold`, base `c8d547f`, was `acd47e5`; gate verdict at `/tmp/gr_annot_verdict.md` | the user's decisions: **margin indicator**, note **ABOVE** the anchored line, **folding**, tree **`C-c a`** (`n`/`h`/`s`/`l`). Gate verified: note-above ordering **with a discriminating test**, aliases dispatching the *same* commands (asserted by identity), margin states (`▎` visible / `▏` folded / none; Grey-on-Black legible and it already carries note text), no weakened assertions (16 removed, all re-expressed; one loop *strengthened*), merge clean with main, fence respected. **P1 found: the `SHIFT` binding is UNREACHABLE, not terminal-conditional** — crossterm 0.29 requires **both** `DISAMBIGUATE_ESCAPE_CODES` and `REPORT_ALL_KEYS_AS_ESCAPE_CODES` for `KeyCode::Modifier`, and iocraft 0.9.1 pushes **only** `REPORT_EVENT_TYPES`, so no terminal (kitty included) can deliver it; the lane's "fires on kitty/WezTerm/foot/Ghostty" claim was false. Repair: unbind SHIFT (the spec said leave it unbound if the flags are off), reframe the handler test, fix the docs (`57442` is LeftControl; RightShift is `57447`). **User-facing loss: `C-c a` no longer toggles — with Shift dead the only working toggles are `C-c a h`/`C-c a s`; `C-c a t` is free if the user wants a single toggle key** |
