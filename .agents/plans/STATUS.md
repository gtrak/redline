...
| Spec | State | Evidence | Notes |
|---|---|---|---|
| `issue-lang-highlight-parity` | IN FLIGHT | lane slot 1, branch `lang-highlight`, base `8ab969e` | user-reported: TypeScript thin inside a function body, C++ not at Rust's level. Measured cause: we pass each grammar's own `HIGHLIGHTS_QUERY` through, so richness is upstream's — Rust **161** lines, C++ **70**, TypeScript **35** (5 capture kinds vs 21). Two fixes: map the captures we silently drop (`@escape` rust/python/go/ruby, `@delimiter` c, `@module` c-sharp — default-coloured in *every* language incl. Rust), and ship superset queries for TypeScript then C++ (confirmed against each grammar's `node-types.json`) |
