...
| 015-04 yank semantics (`issue-015-04-yank-semantics`) | OPEN | `yank` (`src/app/store/buffers.rs:1430`) inserts at the honest point in every mode — no `BufferMode` dispatch: the spec wants Annotation = append at the end (routed through `insert_text`), Accurate = insert at the point, and `M-y` pop replacing at the yank's start under each | the 02 dependency is gone; what remains is the routing + the spec's tests (a)–(e) |
