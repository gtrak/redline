...
| `issue-annotations-fold-visual` | IN FLIGHT | lane slot 1, branch `annot-visual`, base `d665ce3` | the user's follow-up to the landed fold work: **the margin glyph becomes a fold ARROW** (`▾` shown / `▸` folded / nothing unannotated — the bar `▎`/`▏` goes away), a **2-branch tree-line** when open (a diagonal branch up to the note + a straight branch out to the code, arrow as the junction), and **`C-c a h` becomes the TOGGLE with `C-c a s` removed** — which also re-homes the toggle that the bare `C-c a` lost when it became a prefix. **The trap: the code text must not shift horizontally when toggling** (folded `▸ fn` vs expanded `▾─ fn` jitters the whole view on every fold), pinned by a test asserting the code start column is identical in both states. Also: the fold invariants (cursor mid-file, scroll, row-map round-trip, row count) must keep passing, and the PTY drives (which assert the old bar) must be **updated, not loosened** |
