...
every issue in `.agents/plans/*/PLAN.md`. When a lane lands, its state here
...
## Plan 004 — emacs parity (active folder, all issues landed)

| Spec | State | Evidence | Notes |
|---|---|---|---|
| 004-01 isearch interception (`.agents/plans/004-emacs-parity/01-*.md`, task `issue-004-01-impl`) | LANDED | `isearch_key_event` (`src/app/store/keys.rs:75`), `register_isearch` (`src/app/command.rs:378`) | — |
...
## Plan 007 — syntax-aware linking (active folder, all issues landed)
...
## Plan 010 — bundled static analysis (active folder, Shape A complete)
...
## Plan 012 — project organization (active folder; structurally complete)
...
| 013-03 pre-frame harness sub-class | OPEN | **neither deliverable is present.** The pre-frame legs the suite carries (`?25l` counted / no-lingering asserts, `tools/check_cursor_stream.py:8–12,249`) came from plan 004 issue 05 (commit `c964a07`) — *before plan 013 existed* — so they are not evidence this lane ran; and the spec's second deliverable (record the outcome/decision in `00-worklist.md` § Gate reliability class 1) was never written. The evidence points to not-run | `cup_settle` (`check_cursor_stream.py:135`) covers a starved CUP, which the spec explicitly does NOT conflate with the pre-frame class |
...
## Archived plans 001–011 — grouped, **sampled, not audited**
...
> **CAVEAT — read before trusting this section.** These rows are
> **inference, not audit**: ~39 archived specs collapse into 10 rows, and
> for 002 / 003 / 006 a single verified anchor stretches across specs that
> were never individually checked. The residual risk is one-directional and
> real: **an unlanded archived spec could hide inside a LANDED group.** If a
> behavior attributed to one of these groups is missing from the code,
> re-verify that spec individually before believing this table.
...
| 004 (emacs parity) | all (see the 004 table — every spec named there) | LANDED | full table above |
...
| 007 (syntax-aware linking) | all (see the 007 table — every task spec named there) | LANDED | full table above |
...
| 010 (bundled static analysis) | all (see the 010 table — every issue named there) | LANDED | full table above (Shape B / 010-04 is the OPEN-contingent row, not part of this group's LANDED claim) |
...
reliability. They are listed so the "anything else we forgot to work?"
...
- 012 worklist § Gate reliability: the `git::repo` swap-exhaustion flake
