...
impl<'a> Terminal<'a> {
    ...
    /// Stashes the hardware cursor position the app declares for subsequent
    /// frames (see [`TerminalBackend::set_cursor_position`]).
    pub fn set_cursor_position(&mut self, position: Option<(u16, u16)>) {
